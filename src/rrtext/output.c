...
static int
dim_seq(struct node *n)
{
	int w = 0, top = 0, bot = 1;
	struct list *p, **q;

	for (q = &n->u.seq; *q != NULL; q = &(**q).next) {
		if (!node_walk_dim(&(*q)->node)) {
			return 0;
		}
	}
...
node_walk_dim(struct node **n)
{
	...
	switch ((*n)->type) {
	case NODE_SKIP:
		return dim_nothing(*n);

	case NODE_LITERAL:
		return dim_literal(*n);
